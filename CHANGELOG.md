...
- `README.md` with self-hosting guide, CLI docs, architecture overview, and contributing intro.
- `CONTRIBUTING.md` — step-by-step guide for adding scan signals, writing tests, and submitting PRs.
- `LICENSE` (MIT).
- `.gitignore` extended to exclude internal dev notes and test output artifacts.
...
### Added
- Public CLI trust manifest at `/cli/manifest.json` (generated by `npm run build:cli`) with source repository URL, build timestamp, CLI SHA-256, and explicit auth/data-flow guarantees.
- New `/security` page with a plain-language security contract describing exactly what the CLI reads locally, encrypts, and transmits.
- Onboarding verification snippets for checksum validation (PowerShell and macOS/Linux) plus signature-verification instructions tied to manifest signature fields.
...
- CLI auth now accepts `GRAVIO_TOKEN` / `GRAVIO_API_KEY` environment variables and warns when using `--token` due to shell-history exposure risk.
- Onboarding and dashboard command snippets now prefer env-var token usage over `--token` argv usage.
...
- Step 2 now uses a single command with env-var auth: `GRAVIO_TOKEN=<gv_...> node gravio.mjs` (or PowerShell equivalent).
